...
For the purposes of genotype checking, the umi-dedup .bam files were converted back to paired fastq files, then were pseudo-aligned to a modified version of the zebrafish trannscriptome (GRCz11, cDNA , primary assembly). This didnt really work, so I just manually inspected the two sites in the alignent using IGV. 
...
sig_in <- 
  ...
  mutate(sig_in = case_when(
    `T428del/+` == "TRUE" & `W233fs/+` == "TRUE" ~ 'both', 
    `T428del/+` == FALSE & `W233fs/+` == "TRUE" ~ 'fHS only', 
    `T428del/+` == "TRUE" & `W233fs/+` == FALSE ~ 'EOfAD only', 
    TRUE ~ 'neither'
  )) %>% 
...
fgseaRes_nopsen1 <- ranks_fgsea_nopsen1 %>%
  lapply(function(x){
    fgseaMultilevel(stats = x, pathways = c(KEGG, ireGenes), eps = 0) %>%
      as_tibble() %>%
      dplyr::rename(FDR = padj) %>%
      mutate(padj = p.adjust(pval, "bonferroni")) %>%
      dplyr::select(pathway, pval, FDR, padj, everything()) %>%
...
fgseaRes_nopsen1$`fHS-like/+` %<>% 
  mutate(coef = 'fHS-like/+')
...
design %>% colnames() %>% .[2:3] %>% 
  ...
  mutate(Upregulated = NGenes*PropUp, 
         ...
         ns = NGenes - Upregulated - Downregulated) %>% View
  left_join(HMP_notch) %>% 
...
# Check APP AICD targets

I used this review (https://doi.org/10.1016/j.pneurobio.2017.05.005) to get a list of targets for the AICD to assess whether its trascriptional targets are altered. 


```{r}

grGenes %>% as_tibble() %>% dplyr::filter(gene_name %in% c("appa", "appb")) %>%  .$gene_id -> temp
...
sig_in_all <- comp_w_Q96 %>% 
  ...
  mutate(sig_in = case_when(
    ...
    `EOfAD-like/+` == TRUE & `fAI-like/+` == "FALSE" & `Q96_K97del/+` == "FALSE" ~ "T428 only",
